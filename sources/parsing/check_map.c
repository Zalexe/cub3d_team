...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/10 17:31:28 by cmarrued          #+#    #+#             */
/*   Updated: 2025/09/10 17:31:29 by cmarrued         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

//check that there are not weird parameters or too many players
...
//check that player is inside map and not trapped
static int	check_position_is_valid(t_data *data, char **map_tab)
{
	...
	if (ft_strlen(map_tab[i - 1]) < (size_t)j
		...
		|| is_white_space(map_tab[i][j - 1]) == SUCCESS
		|| is_white_space(map_tab[i][j + 1]) == SUCCESS
		|| is_white_space(map_tab[i - 1][j]) == SUCCESS
		|| is_white_space(map_tab[i + 1][j]) == SUCCESS)
		return (FAILURE);
	return (SUCCESS);
}

//check that there is a player with valid direction
...
//check map is put after textures and colors
...
//with parsed map into data, check for errors
